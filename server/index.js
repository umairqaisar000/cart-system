...
app.get("/items", (req, res) => {
  res.send(lineItemsData);
});

app.post("/deliverytime", (req, res) => {
  const { postCode, lineItems } = req.body;
  // Filter delivery dates based on postal code
  const filteredDeliveryDates = DELIVERY_DATES.filter(
    (deliveryDate) => deliveryDate.postal === postCode.charAt(0)
  );

  // Add estimatedDeliveryDate to line items
  const updatedLineItems = lineItems.map((lineItem) => {
    const { id } = lineItem;
    const matchedDeliveryDate = filteredDeliveryDates.find((deliveryDate) =>
      deliveryDate.ids.includes(id)
    );
    return {
      ...lineItem,
      estimatedDeliveryDate: matchedDeliveryDate
        ? matchedDeliveryDate.estimatedDeliveryDate
        : "",
    };
  });
